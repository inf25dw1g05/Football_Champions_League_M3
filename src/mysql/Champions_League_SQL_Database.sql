...
CREATE TABLE IF NOT EXISTS standings (
    ...
    loss INT DEFAULT 0,
    PRIMARY KEY (id),
    CONSTRAINT fk1_team FOREIGN KEY (id) REFERENCES team(id)
);
...
FOR EACH ROW
BEGIN
    INSERT INTO stats (id, total_goals, total_assists, total_yellow_cards, total_red_cards)
...
FOR EACH ROW
BEGIN
    INSERT INTO standings (id, group_name, team_name, points, goal_diff, goals_for, goals_against, played, win, draw, loss)
    VALUES (NEW.id, NEW.group_name, NEW.name, 0, 0, 0, 0, 0, 0, 0, 0);
END $$
...
FOR EACH ROW
BEGIN
    DELETE FROM standings WHERE id = OLD.id;
END $$
