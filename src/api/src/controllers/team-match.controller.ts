...
export class TeamMatchController {
  ...
  async find(
    ...
    return this.teamRepository.homeMatches(id).find(filter);
  }
  ...
  async create(
    @param.path.number('id') id: typeof Team.prototype.team_id,
    @requestBody({
      content: {
        'application/json': {
          schema: getModelSchemaRef(Match, {
            ...
            exclude: ['match_id'],
            optional: ['home_team_id']
          }),
        },
      },
    }) match: Omit<Match, 'match_id'>,
  ): Promise<Match> {
    return this.teamRepository.homeMatches(id).create(match);
  }
  ...
  async patch(
    ...
    return this.teamRepository.homeMatches(id).patch(match, where);
  }
  ...
  async delete(
    ...
    return this.teamRepository.homeMatches(id).delete(where);
  }
}
